...
import { getTotalLeaves, updateLeaveAction } from '../../services';
...
export const LeaveRequest = ({ row, columns }) => {
  const { allLeaves, unapprovedResult, loading } = useSelector(
    (state) => state?.leaves
  );
  ...
  const validateDate = (date) => {
    ...
    return !moment(fromDate).isSameOrAfter(moment().format('l'), 'day');
  };
  ...
  const tabsData = [
    {
      ...
      data: (
        <BasicTable
          ...
          rows={renderLeaveRequest(pending)}
